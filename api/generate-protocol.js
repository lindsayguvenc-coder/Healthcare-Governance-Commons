...
// Calls westgard-tiers, bias-crossref, and commons-node-mapper in parallel.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [tiersResult, biasResult, nodesResult] = await Promise.all([
      callEngine(`${baseUrl}/api/westgard-tiers`, { answers, logicFlags, gapFlags }),
      callEngine(`${baseUrl}/api/bias-crossref`, { answers, logicFlags, gapFlags }),
      callEngine(`${baseUrl}/api/commons-node-mapper`, { answers, logicFlags, gapFlags }),
    ]);
